...
Output: models/relaxation/C_alpha_table_AR{ar_int}.json  (e.g. AR20 for AR=2.0)
...
def main():
    ...
    parser.add_argument(
        "--output", default=None,
        help="Output JSON path. Default: models/relaxation/C_alpha_table_AR{ar_int}.json"
    )
    parser.add_argument(
    ...
    if not os.path.exists(theta_table_path):
        raise FileNotFoundError(
            ...
            f"Run first:  python -m scripts.calibration.extract_theta_target --config {args.config} --AR {AR}"
        )
    ...
    output_path = args.output or \
        config.get('calibration', {}).get(
            'C_alpha_table_file',
            f"models/relaxation/C_alpha_table_AR{ar_int:02d}.json"
        )
    ...
    gmm_path = f"models/exchange_gmm/gmm_cond_AR{ar_int:02d}.npz"
