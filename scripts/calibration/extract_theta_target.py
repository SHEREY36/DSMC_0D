...
Usage:
    python -m scripts.calibration.extract_theta_target --config config/default.yaml --AR 2.0
    python -m scripts.calibration.extract_theta_target --config config/default.yaml --AR 2.0 --alphas 0.80,0.85,0.90
    python -m scripts.calibration.extract_theta_target --config config/default.yaml --AR 2.0 --force

Output: models/targets/theta_target_table_AR20.json  (key: "(alpha, AR)" -> float theta*)
"""
import argparse
import glob
import json
...
def main():
    ...
    parser.add_argument(
        "--output", default=None,
        help="Output JSON path. Default: models/targets/theta_target_table_AR{ar_int}.json"
    )
    parser.add_argument(
    ...
    output_path = args.output or config.get('calibration', {}).get(
        ...
        f"models/targets/theta_target_table_AR{ar_int:02d}.json"
    )
