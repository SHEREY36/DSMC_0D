...
from src.internal.usf.dsmc import run_simulation
...
def build_campaign_config(base_config, AR, alpha, seed, output_root,
    ...
    if AR == 1.0:
        ...
        cfg["preprocessing"]["gmm"]["gmm_cond_file"] = _require_existing(
            model_dir / "exchange_gmm" / f"gmm_cond_{model_label}.npz",
            f"GMM conditional model for AR={AR:g}",
        )
        cfg["calibration"]["C_alpha_table_file"] = _require_existing(
            model_dir / "relaxation" / f"C_alpha_table_{model_label}.json",
            f"C_alpha table for AR={AR:g}",
        )

        ftr_path = _maybe_existing(
            model_dir / "archive" / f"ftr_params_{model_label}_r100.json"
        )
